...
def download_secmmlu():
    ...
    for csv_file in [CS_DEV_CSV, CS_VAL_CSV, CS_TEST_CSV]:
        csv_url = 'https://raw.githubusercontent.com/zefang-liu/cybersecurity-data/' \
            '7d77003027de028cd2daa25e5d03efd07ca09bd7/computer_security/' + csv_file
...
def download_cyquiz():
    ...
    cyquiz_url = 'https://raw.githubusercontent.com/Ebazhanov/linkedin-skill-assessments-quizzes/' \
...
def combine_choices(df_mc):
    ...
    df_mc['choices'] = df_mc.apply(
        lambda row: [row['A'], row['B'], row['C'], row['D']], axis='columns')
    df_mc['answer'] = df_mc.answer.apply(lambda answer: ord(
        answer) - ord('A') if type(answer) == str else answer)
    return df_mc[df_mc.columns.drop(['A', 'B', 'C', 'D'])]


def format_choices(row):
    """
    Format choices as "A. Choice A"
    """
    question = row.question
    choices = '\n'.join(
        [chr(ord('A') + i) + '. ' + choice for i, choice in enumerate(row.choices)])
...
def load_secmmlu_data(mmlu_folder):
    ...
    df_mmlu_cs_dev = pd.read_csv(
        os.path.join(mmlu_folder, CS_DEV_CSV), header=None, names=mmlu_column_names)
    df_mmlu_cs_val = pd.read_csv(
        os.path.join(mmlu_folder, CS_VAL_CSV), header=None, names=mmlu_column_names)
    df_mmlu_cs_test = pd.read_csv(
        os.path.join(mmlu_folder, CS_TEST_CSV), header=None, names=mmlu_column_names)
    ...
    df_secmmlu = pd.concat(
        [df_mmlu_cs_dev, df_mmlu_cs_val, df_mmlu_cs_test], ignore_index=True)
...
def get_df_secmmlu():
    ...
    df_secmmlu = assign_instructions(
        df_secmmlu, outputs=None, instructions=mc_instructions)
    df_secmmlu['task'] = 'mc'
...
def drop_questions(data, verbose=False):
    ...
    for i, (question, choices, answers) in enumerate(zip(data['question'], data['choices'], data['answers'])):
...
def load_cyquiz_data(cyquiz_file_path):
    ...
    """
    with open(cyquiz_file_path, 'r') as file:
    ...
    for line in lines:
        if line.startswith(question_prefix):
            question = '. '.join(line[len(question_prefix):].split('. ')[1:])
            data['question'].append(question)
            data['choices'].append([])
            data['answers'].append([])
        elif line.startswith(choice_prefix):
            if line[len(choice_prefix):len(choice_prefix) + 1] == 'x':
                data['answers'][-1].append(len(data['choices'][-1]))
            choice = line[len(choice_prefix) + 1 + len(choice_sufix):]
...
def get_df_cyquiz():
    ...
    df_cyquiz = assign_instructions(
        df_cyquiz, outputs=None, instructions=mc_instructions)
    df_cyquiz = split_dataset(
        df_cyquiz, train_size=5, val_size=23, test_size=100)
    df_cyquiz['task'] = 'mc'
